...
class AriaDownloadHelper(DownloadHelper):
    ...
    def __onDownloadStarted(self, api, gid):
        ...
        if STOP_DUPLICATE_MIRROR:
            if smsg:
                dl.getListener().onDownloadError(f'Fɪʟᴇ ɪꜱ ᴀʟʀᴇᴀᴅʏ ᴀᴠᴀɪʟᴀʙʟᴇ ɪɴ ᴅʀɪᴠᴇ. Yᴏᴜ ꜱʜᴏᴜʟᴅ ʜᴀᴠᴇ ꜱᴇᴀʀᴄʜ ʙᴇғᴏʀᴇ ᴍɪʀʀᴏʀ ᴀɴʏ ғɪʟᴇ\n\n')
                print(dl.getListener())
                sendMarkup(" Hᴇʀᴇ ᴀʀᴇ ᴛʜᴇ ꜱᴇᴀʀᴄʜ ʀᴇꜱᴜʟᴛꜱ:👇", dl.getListener().bot, dl.getListener().update, button)
    ...
    def __onDownloadPause(self, api, gid):
        ...
        dl = getDownloadByGid(gid)
        dl.getListener().onDownloadError('Dᴏᴡɴʟᴏᴀᴅ ᴄᴀɴᴄᴇʟʟᴇᴅ ʙʏ ʏᴏᴜ ᴏʀ ᴀɴ ᴀᴅᴍɪɴ!')

    @new_thread
    def __onDownloadStopped(self, api, gid):
        LOGGER.info(f"onDownloadStop: {gid}")
        dl = getDownloadByGid(gid)
        if dl: dl.getListener().onDownloadError('Dᴏᴡɴʟᴏᴀᴅ ᴄᴀɴᴄᴇʟʟᴇᴅ ʙʏ ʏᴏᴜ ᴏʀ ᴀɴ ᴀᴅᴍɪɴ!')
